...
class DesgloseIngresosCalculator {
  const DesgloseIngresosCalculator();
  ...
  List<ResumenCategoria> calcular({
    ...
    for (final movimiento in movimientos) {
      ...
      if (!belongsToPeriod || movimiento.tipo != TipoMovimiento.ingreso) {
        continue;
      }
    ...
    final totalIncome = totals.values.fold<int>(0, (sum, total) => sum + total);
    ...
    final breakdown = totals.entries.map((entry) {
      return ResumenCategoria(
        ...
        porcentaje: entry.value * 100 / totalIncome,
      );
    }).toList();
